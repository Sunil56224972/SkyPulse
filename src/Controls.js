...
export class Controls {
  ...
  setupFlightControls(maxFlightCount = 7000) {
    ...
    this.controllers.flightCount = flightFolder
  ...
  /**
   * Update the max flight count on the flight count slider.
   * Called after live data is fetched so the slider reflects actual data size.
   */
  updateMaxFlightCount(newMax) {
    if (this.controllers.flightCount) {
      const folder = this.gui.__folders["Flight Controls"];
      if (folder) {
        folder.remove(this.controllers.flightCount);
        this.guiControls.flightCount = Math.min(newMax, Math.max(1, Math.floor(newMax * 0.5)));
        this.controllers.flightCount = folder
          .add(this.guiControls, "flightCount", 1, newMax, 1)
          .name("Count")
          .onChange((value) => {
            if (this.callbacks.onFlightCountChange) {
              this.callbacks.onFlightCountChange(value);
